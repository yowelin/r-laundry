...
class DataBase {
    ...
    private $querySymbol = "{?}"; // "Символ значения в запросе"
    private $server = "localhost";
    private $user = "id6708612_root";
    private $pass = "Der375HtN";
    private $database = "id6708612_gameseries_db";
    ...
    private function getQuery($query, $params) {
        if ($params) {
            for ($i = 0; $i < count($params); $i++) {
                $pos = strpos($query, $this->querySymbol);
                $arg = "'" . $this->mysqli->real_escape_string($params[$i]) . "'";
                $query = substr_replace($query, $arg, $pos, strlen($this->querySymbol));
            }
        }
        return $query;
    }
    ...
    public function insertRow ($query, $params = false) {
